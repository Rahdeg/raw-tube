                ...
                <CarouselContent className=" -ml-3">
                    {!isLoading && (
                        <CarouselItem className="pl-3 basis-auto">
                            <Badge variant={!value ? "default" : "secondary"} onClick={() => onSelect(null)}
                    ...
                    )}
                    {
                        ...
                        Array.from({ length: 13 }).map((_, index) => (
                            <CarouselItem key={index} className="pl-3 basis-auto">
                                <Skeleton className=" rounde-lg px-3 py-1 h-full text-sm w-[100px] font-semibold">
                                    &nbsp;
                                </Skeleton>
                            </CarouselItem>
                        ))
                    }
                    {
                        !isLoading && data.map((item) => (
                            <CarouselItem key={item.value} className="pl-3 basis-auto">
                                <Badge variant={value === item.value ? "default" : "secondary"} onClick={() => onSelect(item.value)}
                                    className=" rounded-lg px-3 py-1 whitespace-nowrap text-sm cursor-pointer">
                                    {item.label}
                                </Badge>
                            </CarouselItem>
